...
_IOC_DOMAIN_POINTS = 12
_IOC_IP_POINTS = 15
_IOC_SUSPICIOUS_STRING_POINTS = 8
...
def _ioc_risk_score(threat_intelligence: dict[str, Any] | None) -> int:
    ...
    score = (domain_count * _IOC_DOMAIN_POINTS) + (ip_count * _IOC_IP_POINTS) + (
...
def _static_component_steps(static_analysis: dict[str, Any] | None) -> list[dict[str, str]]:
    static_analysis = normalize_static_analysis(static_analysis, source_format="decision_trace")
...
class DecisionTraceEngine:
    ...
    def build(
        ...
        Args:
            static_analysis: Output from StaticAnalyzer.analyze().
            threat_intelligence: Behavior / sandbox intelligence payload.
            final_verdict: Pipeline final_verdict dict (verdict, confidence, scores).

        Returns:
            decision_trace list and final_explanation string.
        """
        static_analysis = normalize_static_analysis(
            static_analysis,
            source_format="decision_trace_engine.build",
        )
        threat_intelligence = threat_intelligence or {}
        ...
        decision_trace.extend(_dynamic_component_steps(threat_intelligence))
        decision_trace.extend(_ioc_component_steps(threat_intelligence))
