...
_FULL_URL_PATTERN = re.compile(r"https?://[^\s\"\'<>]+", re.IGNORECASE)
...
_COMMON_TLDS = (
    ...
    ".co", ".io", ".us", ".uk", ".ru", ".de", ".cn", ".jp", ".fr", ".au",
    ".br", ".in", ".it", ".nl", ".ca", ".kr", ".ch", ".se", ".es", ".xyz",
    ".info", ".biz", ".me", ".tv", ".cc", ".ws", ".bz", ".mobi", ".asia", 
    ".name", ".tel", ".pro", ".aero", ".coop", ".museum", ".top", ".club",
    ".vip", ".fun", ".online", ".site", ".tech", ".store", ".shop", ".app",
    ".dev", ".cloud", ".website", ".space", ".press", ".pw", ".tk", ".ml", ".ga"
)

def _is_plausible_domain(value: str, is_bare: bool = False) -> bool:
    lowered = value.lower()
    ...
    # Exclude <appname>.* if it's clearly a class name and not a standard TLD
    if lowered.startswith("insecurebankv2.") and not lowered.endswith((".com", ".org", ".net", ".io", ".co")):
        return False
    ...
    if is_bare:
        if not any(lowered.endswith(tld) for tld in _COMMON_TLDS):
            return False
...
def extract_iocs(events: list[dict[str, Any]]) -> dict[str, list[str]]:
    ...
    for event in events:
        ...
        for match in _FULL_URL_PATTERN.findall(message):
            url = match.strip()
        ...
        for match in _BARE_DOMAIN_PATTERN.findall(message):
            host = match.strip().lower()
            if _is_plausible_domain(host, is_bare=True) and host not in domains:
                domains.append(host)
