...
FLAG_EVIDENCE_MAP: dict[str, str] = {
    ...
    "SENSITIVE_DATA_ACCESSED": "Application attempted to read sensitive user data (SMS/Contacts/Telephony)",
    "FILE_ACCESS_DETECTED": "Application performed file system or storage operations",
    "DATABASE_ACTIVITY_DETECTED": "Application performed SQLite database or content provider operations",
}
...
def _sample_log_evidence(events: list[dict[str, Any]], event_type: str, limit: int = 2) -> list[str]:
    ...
    for event in events:
        ...
        if len(message) > 140:
            message = message[:137] + "..."
        samples.append(f"[{timestamp}] Log sample [{tag}]: {message}")
...
def build_evidence(
    ...
    if "NETWORK_BURST" in flags:
        ...
        evidence.extend(_sample_log_evidence(events, "network", limit=2))
    elif "network_activity_detected" in flags:
        evidence.extend(_sample_log_evidence(events, "network", limit=1))
        
    if "activity_lifecycle_events_detected" in flags:
        evidence.extend(_sample_log_evidence(events, "activity", limit=1))
...
class BehaviorEngine:
    ...
    def analyze(
        ...
        if event_counts.get("sensitive_data", 0) > 0:
            flags.append("SENSITIVE_DATA_ACCESSED")
        if event_counts.get("file_access", 0) > 0:
            flags.append("FILE_ACCESS_DETECTED")
        if event_counts.get("database_access", 0) > 0:
            flags.append("DATABASE_ACTIVITY_DETECTED")
        ...
        if risk_score > 0 and not evidence_list:
            import logging
            logging.warning(f"PIPELINE BUG: behavior_engine generated risk_score={risk_score} but evidence is empty.")
            evidence_list.append("Warning: Behavioral risk detected but specific evidence logs were unrecoverable.")
